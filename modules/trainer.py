...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        try:
            # 1. 获取原始模型 (处理 DataParallel)
            if isinstance(self.model, torch.nn.DataParallel):
                model_ref = self.model.module
            else:
                model_ref = self.model
            
            # 2. 访问 visual_extractor -> sagnn_adapter -> gate
            # 确保对象存在，避免报错
            if hasattr(model_ref, 'visual_extractor') and \
               hasattr(model_ref.visual_extractor, 'sagnn_adapter') and \
               hasattr(model_ref.visual_extractor.sagnn_adapter, 'gate'):
                
                gate_param = model_ref.visual_extractor.sagnn_adapter.gate
                
                # 获取原始值
                gate_raw = gate_param.item()
                # 获取激活后的值 (假设用的是 tanh，如果是 sigmoid 会显示 sigmoid 后的值)
                gate_active = torch.tanh(gate_param).item()
                
                self.logger.info(
                    'Epoch [{}] SAGNN Gate Status | Raw Value: {:.8f} | Activation (Tanh): {:.8f}'.format(
                        epoch, gate_raw, gate_active)
                )
        except Exception as e:
            self.logger.warning(f"Failed to log gate coefficient: {e}")        
