...
def build_optimizer(args, model):
    # 1. 获取 SAGNN 的参数对象（传给优化器用的）
    sagnn_params = model.visual_extractor.sagnn_adapter.parameters()
    
    # 2. 获取 SAGNN 的参数 ID（仅用于过滤 ResNet 参数用的，是一堆整数）
    sagnn_param_ids = list(map(id, model.visual_extractor.sagnn_adapter.parameters()))
    
    # 3. 筛选 ResNet 参数
    # 逻辑：遍历 visual_extractor 所有参数，如果 ID 不在 sagnn_param_ids 里，那就是 ResNet 的参数
    ve_params = filter(lambda p: id(p) not in sagnn_param_ids, model.visual_extractor.parameters())
    
    # 4. 获取 Encoder-Decoder 参数
    ed_params = model.encoder_decoder.parameters()
    ...
    optimizer = getattr(torch.optim, args.optim)(
        [
            ...
            # SAGNN 部分：正常学习率
            {'params': sagnn_params, 'lr': args.lr_ed},
            
            # Transformer 部分：正常学习率
            {'params': ed_params, 'lr': args.lr_ed}
        ],
        betas=args.adam_betas,
        eps=args.adam_eps,
